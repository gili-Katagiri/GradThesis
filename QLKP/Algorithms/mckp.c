...
void solve(Vector **R){
    ...
    for(i=0;i<GroupNum;i++){ //繰り下げ解の準備 ifs_idx以外の和
        if( i==ifs_idx ) continue;
        Node *node = (Node*)(vec[i]->data);
        P_cp += node->item->profit;
    }
    
    if( ifs_idx == GroupNum ){ //X_CP_r is integer?
        update(vec,NULL,GroupNum);
    ...
    if( debug ){
        printf("\nSeparate :\nRight Side--------------------\n");
        for(i=0;i<GroupNum;i++){ print_nodes(R[i]); putchar('\n'); }
        printf("Left Side---------------------\n");
        for(i=0;i<GroupNum;i++){ print_nodes(R_left[i]); putchar('\n'); }
        printf("------------------------------\n\n");
    }
    //calc_penalty pq+ , pq-
    ...
    if( debug ){ printf("P_cpr="); print_frac(P_cpr); printf(", P_inc=%llu\n\n",P_inc); }
...
Vector** MCKP_prepare(void){
    ...
    Rk = (Vector**)malloc(GroupNum*sizeof(Vector*));
    for( i=0;i<GroupNum;i++){
        Rk[i] = NULL;
        //vec_ptr = Cand_List[i]; //Candidate List
...
Vector** Algorithm1(Vector **R){
    ...
    Fraction *min;
    Vector **vec;
    vec = (Vector**)malloc(GroupNum*sizeof(Vector*));
    for( i=0;i<GroupNum;i++ ){
        vec[i] = R[i];
    ...
    while(1){
        ...
        for( i=0;i<GroupNum;i++ ){ //傾き最小を見つける
        ...
        if( min_idx==-1 ){ //解なし all x->0 総価値が0という扱い Rk_feasibleでは-1が返る
            ...
            for(i=0;i<GroupNum;i++){ ((Node*)(vec[i]->data))->x = frac_zero; }
            return vec;
        }
...
Vector** relaxation2(Vector **R,Fraction *border,Node *right,int group){
    ...
    R_left = (Vector**)malloc(GroupNum*sizeof(Vector*));
    for(i=0;i<GroupNum;i++){
        if( i==group ) continue;
        R_left[i] = copy_R(R[i]);
...
void rx_left(Vector **R,Fraction *border, int group){
    Vector *Nk=Cand_List[group], *left_add=NULL;
